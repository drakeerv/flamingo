 ...
 * Note that this does NOT set up the runtime environment (scopes); that happens in `flamingo_run` or `flamingo_inherit_env`.
 * Note the `src` buffer is NOT copied. It must remain valid for the lifetime of the flamingo instance.
 *
 ...
 * This means that subsequent calls to `flamingo_run` on the same instance will reset the state.
 *
 * @param flamingo The flamingo instance.
 * @return 0 on success, -1 on error.
 */
