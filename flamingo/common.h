 ...
 *
 * This allocates a new scope and attaches it. The new scope inherits the `class_scope` property from the parent.
 *
 * @param env The environment.
 ...
 * Free a scope.
 *
 * This frees the scope and all its variables. It should only be called when the reference count reaches 0.
 *
...
/**
 * Find a variable in a scope (shallow search).
 *
 ...
 *
 * The value is initialized with `val_init`.
 *
 ...
 * Free a value.
 *
 * This frees the value and all its contents. It should only be called when the reference count reaches 0.
 *
